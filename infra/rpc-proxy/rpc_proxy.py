...
TIP_BUFFER = {
    "gnosis":  int(os.environ.get("RPC_PROXY_TIP_BUFFER_GNOSIS",  "20")),
...
class RPCProxyHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):  # noqa: N802 (BaseHTTPRequestHandler convention)
        ...
        # Check the hash pin cache first.
        pin_key = pin_key_for_request(request)
        if pin_key is not None and self.cache is not None:
